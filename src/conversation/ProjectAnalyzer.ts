...
export class ProjectAnalyzer {
  ...
  async analyzeProject(): Promise<ProjectContext> {
    ...
    const fileContents = await this.fileReader.execute({
      paths: keyFiles,
    });
  ...
  private identifyKeyFiles(structure: string): string[] {
    ...
    return keyFiles.slice(0, 15); // Limit to prevent overwhelming the context
  }
  ...
  private identifyMainAppFiles(structure: string): string[] {
    ...
    if (structure.includes('src/index.js') || structure.includes('src/index.jsx')) {
      appFiles.push('src/index.js', 'src/index.jsx');
    }
    if (structure.includes('src/index.ts') || structure.includes('src/index.tsx')) {
    ...
    // Angular patterns
    if (structure.includes('src/main.ts')) appFiles.push('src/main.ts');
    if (structure.includes('src/app/app.module.ts')) appFiles.push('src/app/app.module.ts');
    if (structure.includes('src/app/app.component.ts')) appFiles.push('src/app/app.component.ts');
  ...
  private analyzeProjectContent(structure: string, fileContents: string): {
    techStack: string[];
    patterns: string[];
  } {
    ...
    // Analyze file contents for more specific patterns
  ...
  private detectTechStackFromStructure(
    ...
    if (structure.includes('package.json')) {
      ...
      if (structure.includes('src/App.jsx') || structure.includes('src/App.tsx')) {
        techStack.add('React');
        patterns.add('React Application');
      }
      if (structure.includes('pages/') || structure.includes('next.config.js')) {
    ...
    if (structure.includes('requirements.txt') || structure.includes('setup.py')) {
    ...
    if (structure.includes('vite.config.js') || structure.includes('vite.config.ts')) {
      techStack.add('Vite');
    }
    ...
    // Testing
    if (structure.includes('jest.config.js') || structure.includes('__tests__/')) {
      techStack.add('Jest');
  ...
  private detectTechStackFromContent(
    ...
    if (fileContents.includes('"passport"') || fileContents.includes('"auth0"') || 
        fileContents.includes('"firebase/auth"') || fileContents.includes('"jsonwebtoken"')) {
    ...
    if (fileContents.includes('"/api/"') || fileContents.includes('app.get(') || 
        fileContents.includes('app.post(')) {
      patterns.add('REST API');
    }
    if (fileContents.includes('"apollo"') || fileContents.includes('"graphql"')) {
      techStack.add('GraphQL');
      patterns.add('GraphQL API');
    }

    // State management
    if (fileContents.includes('"redux"') || fileContents.includes('"@reduxjs/toolkit"')) {
      techStack.add('Redux');
      patterns.add('State Management');
    }
    if (fileContents.includes('"zustand"') || fileContents.includes('"jotai"')) {
      patterns.add('State Management');
    }
